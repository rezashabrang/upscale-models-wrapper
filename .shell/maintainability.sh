...
radon mi upscale_wrapper -s >>.logs/maintainability.txt
export maintainability=$(radon mi upscale_wrapper -s | grep -Po '(?<=\().*(?=\))' | perl -lane '$a+=$_ for(@F);$f+=scalar(@F);END{print $a/$f}' | xargs printf "%.1f%%\n")
